<div class="  w-full h-full">
        ...
        <form wire:submit='save' class="w-full flex flex-col md:flex-row h-full md:justify-between md:items-start  gap-1">
            <div x-data="{ $wire.selectedTab }" class="w-full">
                <div x-on:keydown.right.prevent="$focus.wrap().next()" x-on:keydown.left.prevent="$focus.wrap().previous()" class="flex gap-2 overflow-x-auto border-b border-outline dark:border-outline-dark" role="tablist" aria-label="tab options">
                    <button x-on:click="$wire.selectedTab = 'photo'" x-bind:aria-selected="$wire.selectedTab === 'photo'" x-bind:tabindex="$wire.selectedTab === 'photo' ? '0' : '-1'" x-bind:class="$wire.selectedTab === 'photo' ? 'font-bold text-primary border-b-2 border-primary dark:border-primary-dark dark:text-primary-dark' : 'text-on-surface font-medium dark:text-on-surface-dark dark:hover:border-b-outline-dark-strong dark:hover:text-on-surface-dark-strong hover:border-b-2 hover:border-b-outline-strong hover:text-on-surface-strong'" class="h-min px-4 py-2 text-sm" type="button" role="tab" aria-controls="tabpanelphoto" >photo</button>
                    <button x-on:click="$wire.selectedTab = 'text'" x-bind:aria-selected="$wire.selectedTab === 'text'" x-bind:tabindex="$wire.selectedTab === 'text' ? '0' : '-1'" x-bind:class="$wire.selectedTab === 'text' ? 'font-bold text-primary border-b-2 border-primary dark:border-primary-dark dark:text-primary-dark' : 'text-on-surface font-medium dark:text-on-surface-dark dark:hover:border-b-outline-dark-strong dark:hover:text-on-surface-dark-strong hover:border-b-2 hover:border-b-outline-strong hover:text-on-surface-strong'" class="h-min px-4 py-2 text-sm" type="button" role="tab" aria-controls="tabpaneltext" >text</button>
                </div>
                <div class="p-2 text-on-surface dark:text-on-surface-dark">
                    <div x-cloak x-show="$wire.selectedTab === 'photo'" id="tabpanelphoto" role="tabpanel" aria-label="photo">
                    ...
                    <div x-cloak x-show="$wire.selectedTab === 'text'" id="tabpaneltext" role="tabpanel" aria-label="text">
            ...
            <div class="mx-auto p-2 w-full">
                <div class="flex flex-col gap-2 overflow-y-auto rounded-lg scrollbar-thin scrollbar-thumb-outline scrollbar-track-transparent">
                    ...
                    <label
                        ...
                        class="group relative flex w-full min-w-52 items-center justify-start gap-3 rounded-lg border border-outline bg-surface-alt px-4 py-3 font-medium text-on-surface cursor-pointer transition-all duration-300 ease-in-out
                            ...
                            hover:dark:border-primary-dark/70"
                    >
                        <input wire:model="model" value="PreparedDishAnalyzer" type="radio" name="radioPlatform" class="hidden peer">
                        ...
                        <span class="text-sm ml-2 o peer-checked:text-primary dark:peer-checked:text-primary-dark transition-colors duration-200">
                            {{ __("Prepared Dish Analyzer") }}
                        </span>
